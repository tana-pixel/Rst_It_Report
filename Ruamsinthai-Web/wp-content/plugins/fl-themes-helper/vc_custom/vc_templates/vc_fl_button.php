...
if ( ! function_exists( 'vc_fl_render_fl_btn' ) ) {
    function vc_fl_render_fl_btn($atts, $content = null)
    {
        ...
        $button_container_css_classes[] = 'button-container ';

        $btn_css_classes[] = 'fl-vc-button fl-font-style-bolt-two';
        ...
        // Button Size
        if(isset($size) && $size != 'normal') {
            $button_container_css_classes[] = $size;
        }
        // Button Style
        if(isset($btn_style) && $btn_style != 'custom-colors') {
            $button_container_css_classes[] = $btn_style;
        }
        ...
        if(isset($btn_style) && $btn_style == 'custom-colors') {
            ...
            }elseif (! empty( $btn_bg ) && ($btn_bg !='') && ! empty( $btn_hr_animation ) && ($btn_hr_animation =='disable')){
                $fl_helping_css_style[] = '.' . $custom_html_class . ':not([data-animation="true"]) { background-color:' . $btn_bg . '!important; }';
            }
            // Hover Color
            if ( ! empty( $btn_hv_bg ) && ($btn_hv_bg !='') && ! empty( $btn_hr_animation ) && ($btn_hr_animation !='disable')) {
                $fl_helping_css_style[] = '.' . $custom_html_class . '[data-animation] .fl-vc-button:after { background-color:' . $btn_hv_bg . '!important; }';
            }elseif (! empty( $btn_hv_bg ) && ($btn_hv_bg !='') && ! empty( $btn_hr_animation ) && ($btn_hr_animation =='disable')){
                $fl_helping_css_style[] = '.' . $custom_html_class . ':not([data-animation="true"]):hover { background-color:' . $btn_hv_bg . '!important; }';
            }
        }
        ...
        $button_container_css_class = preg_replace( '/\s+/', ' ', implode( ' ', array_filter( array_unique( $button_container_css_classes ) ) ) );


        $result .= '<div class="' . esc_attr( trim( $css_class ) ) .' " '. implode( ' ', $wrapper_attributes ).'>';

            $result .= '<div class="' . esc_attr( trim( $button_container_css_class ) ) .' " '. implode( ' ', $button_container_wrapper_attributes ).'>';
