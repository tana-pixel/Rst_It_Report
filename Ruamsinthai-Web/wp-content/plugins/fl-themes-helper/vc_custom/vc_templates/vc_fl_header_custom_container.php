<?php
if ( ! function_exists( 'vc_fl_header_custom_container_function' ) ) {
    function vc_fl_header_custom_container_function($atts, $content = null)
    {
        $css_classes[] = 'header-bg-container';

        global $fl_helping_responsive_style;

        $atts = vc_map_get_attributes('vc_fl_header_custom_container', $atts);
        ...
        $result .= '<div class="fl-header-custom-content">';
            $result .='<div class="' . esc_attr( trim( $css_class ) ) . '" '.implode( ' ', $wrapper_attributes).'>';
                $result .='<div class="custom-header-container">';
                    $result .='<div class="container">';
                        $result .='<div class="row custom-header-row">';
                            $result .=  '<div class="col-12">'.do_shortcode($content).'</div>';
                        $result .='</div>';
                    $result .='</div>';
                $result .='</div>';
            $result .='</div>';
...
add_shortcode('vc_fl_header_custom_container', 'vc_fl_header_custom_container_function');