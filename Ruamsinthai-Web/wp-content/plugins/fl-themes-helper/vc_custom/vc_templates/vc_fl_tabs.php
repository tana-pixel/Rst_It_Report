...
if ( ! function_exists( 'fl_tabs' ) ) {
    function fl_tabs( $atts, $content = null ) {
        ...
        $result .= '<div class="tab-content col-lg-6">';
        ...
        $result .= '<ul class="nav-tabs '.$ul_css_class.' col-lg-6">';
        foreach( $fl_global_tabs as $key => $tab) {
            $i++;
            if($key == 0){
                $title_html = 'Tab 1';
            ...
            $icon=$custom_html_class='';
            if(isset( $tab['atts']['icon_type'] ) && $tab['atts']['icon_type'] != 'none'){
                switch ($tab['atts']['icon_type']) {
                    case 'fontawesome':
                        $icon = $tab['atts']['icon_fontawesome'];
                        break;
                    case 'openiconic':
                        $icon = $tab['atts']['icon_openiconic'];
                        break;
                    case 'typicons':
                        $icon = $tab['atts']['icon_typicons'];
                        break;
                    case 'entypo':
                        $icon = $tab['atts']['icon_entypo'];
                        break;
                    case 'linecons':
                        $icon = $tab['atts']['icon_linecons'];
                        break;
                    case 'elusive':
                        $icon = $tab['atts']['icon_elusive'];
                        break;
                    case 'etline':
                        $icon = $tab['atts']['icon_etline'];
                        break;
                    case 'iconmoon':
                        $icon = $tab['atts']['icon_iconmoon'];
                        break;
                    case 'linearicons':
                        $icon = $tab['atts']['icon_linearicons'];
                        break;
                    case 'flicon':
                        $icon = $tab['atts']['icon_flicon'];
                        break;
                    case 'iconic':
                        $icon = $tab['atts']['icon_iconic'];
                        break;
                    case 'flaticon':
                        $icon = $tab['atts']['icon_flaticon'];
                        break;
                }

                vc_icon_element_fonts_enqueue($tab['atts']['icon_type']);


            }
            $icon_vc = '<i class="tabs-icon ' . $icon . '"></i>';
            ...
            $result .= '<li class="col-md-6 '.$custom_html_class.'" data-tab="fl-'.$tabunidtab.'-'.$key.'">';
