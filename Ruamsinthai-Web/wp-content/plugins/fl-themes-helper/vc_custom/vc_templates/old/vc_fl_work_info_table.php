 ...
 * Shortcode Partner
 * */
if ( ! function_exists( 'vc_fl_work_info_table_function' ) ) {
    function vc_fl_work_info_table_function($atts, $content = null)
    {
        $css_classes[] = 'fl-work-info-wrapper';

        global $fl_helping_responsive_style;

        $atts = vc_map_get_attributes('vc_fl_work_info_table', $atts);
        ...
        if(isset($custom_responsive_option) && $custom_responsive_option !='off') {
            if( !empty( $responsive_css ) && $responsive_css != '' ) {
                $responsive_id = $idf = uniqid('fl-responsive-work-info').'-'.rand(100,9999);
