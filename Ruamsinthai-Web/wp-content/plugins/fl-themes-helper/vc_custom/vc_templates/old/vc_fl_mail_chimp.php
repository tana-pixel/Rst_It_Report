 ...
 * Shortcode team
 * */
if ( ! function_exists( 'vc_fl_mail_chimp_function' ) ) {
    function vc_fl_mail_chimp_function($atts, $content = null)
    {
        $css_classes[] = 'fl--subscribe-wrapper cf';

        global $fl_helping_responsive_style;

        $atts = vc_map_get_attributes('vc_mail_chimp', $atts);

        extract($atts);

        $result = $wrapper_attributes[]  =  $responsive_style ='';

        if(isset($mailchimp_title) && $mailchimp_title!=''){
            $title = $mailchimp_title;
        }
        ...
        if(isset($custom_responsive_option) && $custom_responsive_option !='off') {
            if( !empty( $responsive_css ) && $responsive_css != '' ) {
                $responsive_id = $idf = uniqid('fl-helping-mail-chimp-responsive-').'-'.rand(100,9999);
        ...
        $result .= '<div class="' . esc_attr( trim( $css_class ) ) . '" '. implode( ' ', $wrapper_attributes ).'>';
                    ...
                    $result .= do_shortcode( '[mc4wp_form id="' .$form_id_mail_chimp. '"]' );
...
add_shortcode('vc_mail_chimp', 'vc_fl_mail_chimp_function');