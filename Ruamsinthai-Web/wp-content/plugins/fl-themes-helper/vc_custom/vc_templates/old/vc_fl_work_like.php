 ...
 * */
if ( ! function_exists( 'vc_fl_work_like_function' ) ) {
    function vc_fl_work_like_function($atts, $content = null)
    {
        $css_classes[] = 'fl-li-work-info cf';

        global $fl_helping_responsive_style;

        $atts = vc_map_get_attributes('vc_fl_work_like', $atts);
        ...
        $result .= '<div class="fl-left-content fl-work-info-title fl-text-title-style">' . $icon_vc . $li_title . '</div>';

        $result .= '<div class="fl-right-content fl-work-info-content">'.get_simple_likes_button( get_the_ID() ).'</div>';
...
add_shortcode('vc_fl_work_like', 'vc_fl_work_like_function');
