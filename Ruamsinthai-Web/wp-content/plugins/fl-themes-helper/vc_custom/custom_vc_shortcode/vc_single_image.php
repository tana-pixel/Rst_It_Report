...
$title = $source = $image = $custom_src = $onclick = $img_size = $external_img_size = $css_classes[] = $wrapper_attributes[] = $mask = $bg_mask = $img_hover_effects =
$style = $responsive_style =  $img_link_large = $link = $img_link_target = $alignment = $el_class = $el_id =$css = '';
...
$wrapperClass = 'vc_single_image-wrapper ' . $style . ' '.$img_hover_effects .' ';

// Image Mask
    if ( ! empty( $mask_background ) && $mask_background != '' ) {
        $bg_mask = 'background:'.$mask_background.';';
    }

    $style_mask = ( $bg_mask ) ? 'style=' . $bg_mask . '' : '';


if ( ! empty( $image_mask ) && $image_mask != 'off' ) {
    $mask .= '<div class="fl-single-image--mask" '. $style_mask.'></div>';
}
