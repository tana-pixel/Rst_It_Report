jQuery.noConflict()(function($) {
    ...
    var initLoadMoreFunction = function() {
        var body = $('body');
        var page_num = parseInt(autlines_pagination_data.startPage) + 1;
        var max_pages = parseInt(autlines_pagination_data.maxPages);
        var next_link = autlines_pagination_data.nextLink;

        var container = autlines_pagination_data.container;
        var $container = $(container);
        var container_has_isotope = false;


        var last_btn_text = autlines_pagination_data.button_text_no_post;
        ...
        if($container.hasClass('fl--blog-post-div')){
            last_btn_text = autlines_pagination_data.button_text_no_post;
        }
        // Last Page Woo Text
        if($container.hasClass('fl--work--wrapper')){
            last_btn_text = autlines_pagination_data.button_text_no_work;
        }
        // Last Page Woo Text
        if($container.hasClass('products')){
            last_btn_text = autlines_pagination_data.button_text_no_product;
        }
        //
        if($container.hasClass('vc-cars-wrapper')){
            last_btn_text = autlines_pagination_data.button_car_text_no_car;
        }
            ...
            if (!$(this).hasClass('loading') && !$(this).hasClass('load_more_disable') && page_num <= max_pages) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $button.text(autlines_pagination_data.button_loading);
                    },
                    complete: function(content) {
                        $button.text(autlines_pagination_data.button_text);
