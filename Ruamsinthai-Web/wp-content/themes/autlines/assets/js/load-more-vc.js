jQuery.noConflict()(function($) {
    ...
    var initLoadMoreFunctionVc = function() {
        var body = $('body');
        var page_num = parseInt(autlines_vc_pagination_data.startPage) + 1;
        var max_pages = parseInt(autlines_vc_pagination_data.maxPages);
        var next_link = autlines_vc_pagination_data.nextLink;

        var container = autlines_vc_pagination_data.container;
        var $container = $(container);
        var container_has_isotope = false;


        var last_btn_text = autlines_vc_pagination_data.button_text_no_post;
            ...
            if (!$(this).hasClass('loading') && !$(this).hasClass('load_more_disable') && page_num <= max_pages) {
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $button.text(autlines_vc_pagination_data.button_loading);
                    },
                    complete: function(content) {
                        $button.text(autlines_vc_pagination_data.button_text);
