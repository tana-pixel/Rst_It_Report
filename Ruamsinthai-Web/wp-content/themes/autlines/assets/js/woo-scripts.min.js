...
jQuery.noConflict()(function($){"use strict";var initCompareCustomFunction=function()
{$(document).on('click','a.compare:not(.added)',function(e){e.preventDefault();var $this=$(this);$this.addClass('loading');$this.closest('.product-inner').find('.compare:not(.loading)').trigger('click');var compare=false;if($(this).hasClass('added')){compare=true;}
if(compare===false){var compare_counter=$(document).find('#top-cart-compare-count').html();compare_counter=parseInt(compare_counter,10)+1;$this.removeClass('loading');setTimeout(function(){$(document).find('#top-cart-compare-count').html(compare_counter);},2000);}});$(document).find('.compare-list').on('click','.remove a',function(e){e.preventDefault();var compare_counter=$('#top-cart-compare-count',window.parent.document).html();compare_counter=parseInt(compare_counter,10)-1;if(compare_counter<0){compare_counter=0;}
...
if(filter_div.hasClass("enable")){filter_div.removeClass('enable');filter_div.addClass('disable');filter_div.slideUp(200);}else{filter_div.slideDown();filter_div.removeClass('disable');filter_div.addClass('enable');}});};var initSliderSingleShopFunction=function(){var slider=$('.product-slider');slider.slick({arrows:false,dots:false,infinite:false,speed:700,slidesToShow:1,slidesToScroll:1,draggable:true,focusOnSelect:true,asNavFor:'.product-carousel'});$('.product-carousel').slick({dots:false,infinite:false,draggable:true,speed:300,slidesToShow:4,slidesToScroll:1,focusOnSelect:true,arrows:false,asNavFor:'.product-slider',responsive:[{breakpoint:1024,settings:{slidesToShow:3,slidesToScroll:1}},{breakpoint:600,settings:{slidesToShow:2,slidesToScroll:1}},{breakpoint:480,settings:{slidesToShow:1,slidesToScroll:1}}]});};var initProductCategoryWidgetAnimation=function(){var $widgetWrapper=$('.widget_product_categories');$widgetWrapper.find('ul.product-categories li.cat-parent').first().addClass('opened');$widgetWrapper.find('li').each(function(){var $this=$(this);if($this.length>0){$this.prepend("<i></i>");}});function toogleWidgetSub_menu($widget_sub_menu_find){var $widget_navigation_Items=$widget_sub_menu_find.find('.cat-parent >.children >li >a,.cat-parent >.children >li');if(!$widget_sub_menu_find.find('.cat-parent >.children >li.opened').length){$widget_navigation_Items=$widget_sub_menu_find.find('.children >li ');}
console.log($widget_navigation_Items);TweenMax.set($widget_navigation_Items,{opacity:0,x:'-20%',force3D:true},0.04);if($widget_sub_menu_find.hasClass('opened')){$widget_sub_menu_find.removeClass('opened');$widget_sub_menu_find.find('li').removeClass('opened');$widget_sub_menu_find.find('ul').slideUp(200);TweenMax.staggerTo($widget_navigation_Items,0.3,{opacity:0,x:'-20%',force3D:true},0.04);console.log($widget_navigation_Items);}else{TweenMax.staggerTo($widget_navigation_Items,0.3,{x:'0%',opacity:1,delay:0.2},0.04);$widget_sub_menu_find.addClass('opened');if(!$widget_sub_menu_find.children('ul').length){$widget_sub_menu_find.find('div').children('ul').slideDown();}else{$widget_sub_menu_find.children('ul').slideDown();}
