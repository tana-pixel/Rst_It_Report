...
class Autlines_Dashboard{
    ...
    public function edit_tgmpa_notice_action_links( $action_links ) {
        ...
        if ( 'tvk-tgm-plugin-install' == $current_screen->id ) {
            $link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="#tvk-tgm-plugin-install">' . esc_attr__( 'Manage Plugins Below', 'autlines' ) . '</a>';
        } else  {
            $link_template = '<a id="manage-plugins" class="button-primary" style="margin-top:1em;" href="' . esc_url( self_admin_url( 'admin.php?page=tvk-tgm-plugin-install' ) ) . '">' . esc_attr__( 'Manage Plugins', 'autlines' ) . '</a>';
        }
    ...
    public function dashboard_init_data(){
        ...
        $this->strings = array(
            ...
            ///
            'dashboard_title'                       => esc_html__('%1$s %2$s', 'autlines'),
            'theme_activation_title'                => esc_html__('Theme activation', 'autlines'),
            'theme_activated'                       => esc_html__('Theme activated', 'autlines'),
            'theme_not_activated'                   => esc_html__('Theme not activated', 'autlines'),
            'theme_link_text'                       => esc_html__('Theme Link', 'autlines'),
            'theme_link'                            => esc_url('http://preview.themeforest.net/item/autlines-automotive-wordpress-theme/full_screen_preview/24858019'),
            'theme_doc_text'                        => esc_html__('Online Documentation', 'autlines'),
            'theme_doc_link'                        => esc_url('https://templines.com/autlines-documentation/'),
            'theme_support_text'                    => esc_html__('Support Center', 'autlines'),
            'theme_support_link'                    => esc_url('https://support.templines.com/'),
            'thx_for_buying'                        => esc_html__('Thank you for purchasing our theme. If you liked the our theme please put 5 stars rating to it.', 'autlines'),
            'footer_thank_you'                      => esc_html__('Thank you for choosing %s!', 'autlines'),
            'widget_more_info_text'                 => esc_html__('More Info', 'autlines'),
            'widget_theme_system_title'             => esc_html__('System Information', 'autlines'),
            'widget_requirements_title'             => esc_html__('Requirements', 'autlines'),
            'widget_requirements_problems'          => esc_html__('Some Problems', 'autlines'),
            'widget_requirements_noproblems'        => esc_html__('No Problems', 'autlines'),


            'plugin_link'                           => esc_url('admin.php?page=tvk-tgm-plugin-install'),


            'remember_code_text'                    => esc_html__('Reminder ! One registration per Website. If registered elsewhere please remove purchased code that registration first.', 'autlines'),
            'code_text'                             => esc_html__('Your code is:', 'autlines'),

            'activation_doc_link'                   => esc_url('http://support.templines.com/knowledge-base/where-can-i-find-my-purchase-code/'),
            'activation_text_left'                  => esc_html__('You can learn how to find your purchase code', 'autlines'),
            'activation_text_right'                 => esc_html__('You  will can later deactivate this code if you use dev site and using again on live version.', 'autlines'),

            'theme_support_title'                   => esc_html__('Theme Support', 'autlines'),
            'support_message'                       => esc_html__('If you did not find the question interest in our documentation, found an error, or if you want to suggest something, you can contact technical support.','autlines'),
            'support_btn_text'                      => esc_html__('Visit Support Center','autlines'),

            'theme_documentation_title'             => esc_html__('Theme Documentation','autlines'),
            'theme_documentation_text'                        => esc_html__('If you have any problems, you can look at the online documentation of this topic, if you have not found a question that interests you, visit our support forum and ask us a question that interests you, we will be happy to help you','autlines'),


            'our_info_title'                        => esc_html__('Our Information','autlines'),
            'out_info_text'                         => esc_html__('You can find out more about our works or our team just follow one of the links below.','autlines')
            );
    ...
    public function edit_menu_title_plugin( $args ) {
        ...
        if ( $count > 0 ) {
            $args['menu_title'] = __( 'Install Plugins', 'autlines' ) . ' <span class="update-plugins"><span class="update-count">' . $count . '</span></span>';
        }
    ...
    public function dashboard_scrips( ) {
        $listPage = [
            ...
            'autlines_page_tvk-system-info',
            'autlines_page_pt-one-click-demo-import',
            'autlines_page_tvk-tgm-plugin-install',
        ];
        if ( in_array( get_current_screen()->base, $listPage ) ) {
            remove_all_actions( 'admin_notices' );
            wp_enqueue_style('autlines-dashboard-css', get_template_directory_uri() . '/admin/theme-dashboard/css/style.css', array(), '1.0');
        }
    ...
    public function create_admin_menus() {
        ...
        add_submenu_page( 'tvk-theme-dashboard',
            esc_html__('System Info', 'autlines'),
            esc_html__('System Info', 'autlines'),
            'manage_options',
            'tvk-system-info',
    ...
    private function setup_tab_menus() {
        ...
        if ( class_exists('OCDI_Plugin') ) {
            $this->tabs_menu['demos'] = [
                'name' => esc_html__( 'Demo Install', 'autlines' ),
        ...
        $this->tabs_menu['customize'] = [
            'name' => esc_html__( 'Customize', 'autlines' ),
            'path'  => 'customize.php',
        ];
    ...
    public function edit_admin_menus() {
        ...
        if ( current_user_can( 'edit_theme_options' ) ) {
            $submenu['tvk-theme-dashboard'][0][0] = esc_attr__( 'Welcome', 'autlines' );
        }
    }
    ...
    public function custom_menu_import() {
        ...
        if (class_exists('OCDI_Plugin')){
            function autlines_plugin_page_setup( $default_settings ) {
                $default_settings['parent_slug'] = 'tvk-theme-dashboard';
                $default_settings['page_title']  = esc_html__( 'Demo Import' , 'autlines' );
                $default_settings['menu_title']  = esc_html__( 'Demo Install' , 'autlines' );
                $default_settings['capability']  = 'import';
                $default_settings['menu_slug']   = 'pt-one-click-demo-import';

                return $default_settings;
            }
            add_filter( 'pt-ocdi/plugin_page_setup', 'autlines_plugin_page_setup' );
        }
    ...
    public static function getInstance() {
        if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Autlines_Dashboard ) ) {
            self::$instance = new Autlines_Dashboard();
        }
...
Autlines_Dashboard::getInstance();
