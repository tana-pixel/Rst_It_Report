<?php
if ( ! function_exists( 'autlines_fontawesome_menu_icons' ) ) :
    function autlines_fontawesome_menu_icons() {
...
if (!class_exists('AUTLINES_Mega_menu')) {
	class AUTLINES_Mega_menu {
		...
		public static function options() {
            $icon_ListMenu = autlines_fontawesome_menu_icons();


			return array(
			    '_autlines_mega_menu_icon'	=> array(
                    'type'      => 'select_icon',
                    'label'     => esc_html__('Select Icon', 'autlines'),
                    'default'   => 0,
                    'options'   => $icon_ListMenu,
                    'size'      => 'wide',
                ),
				'_autlines_mega_menu_image'	=> array(
						'type' => 'upload',
						'label' => esc_html__('Image', 'autlines'),
						'default' => '',
						'size' => 'wide',
						'depth' => '0',  
						'class' => 'autlines-show-only-depth-0',
					),
				
				'_autlines_mega_menu_bg_position'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Background position', 'autlines' ),
						'default' => 0,
						'options' => array(
								'left top'              => esc_html__('Left top', 'autlines'),
								'left center'           => esc_html__('Left center', 'autlines'),
								'left bottom'           => esc_html__('Left bottom', 'autlines'),
								'right top'             => esc_html__('Right top', 'autlines'),
								'right center'          => esc_html__('Right center', 'autlines'),
								'right bottom'          => esc_html__('Right bottom', 'autlines'),
								'center top'            => esc_html__('Center top', 'autlines'),
								'center center'         => esc_html__('Center center', 'autlines'),
								'center bottom'         => esc_html__('Center bottom', 'autlines')
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'autlines-show-only-depth-0',
					),
				'_autlines_mega_menu_bg_repeat'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Background repeat', 'autlines' ),
						'default' => 'no-repeat',
						'options' => array(
								'no-repeat'         => esc_html__( 'No-repeat', 'autlines' ),
								'repeat'            => esc_html__( 'Repeat', 'autlines' ),
								'repeat-x'          => esc_html__( 'Repeat-x', 'autlines' ),
								'repeat-y'          => esc_html__( 'Repeat-y', 'autlines' ),
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'autlines-show-only-depth-0',
					),
				'_autlines_mega_menu_enabled'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Enable mega menu', 'autlines' ),
						'default' => 0,
						'options' => array(
						    1=> esc_html__( 'Yes', 'autlines' ),
                            0=> esc_html__( 'No', 'autlines' )),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'autlines-show-only-depth-0 autlines-mega-menu',
					),
				'_autlines_full_width_menu_enabled'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Enable full-width menu', 'autlines' ),
						'default' => 0,
						'options' => array(
						    1=> esc_html__( 'Yes', 'autlines' ),
                            0=> esc_html__( 'No', 'autlines' )),
				...
				'_autlines_mega_menu_limit_columns'	=> array(
						'type' => 'select',
						'label' => esc_html__( 'Limit Max columns number', 'autlines' ),
						'default' => 0,
						'options' => array(
								''	=>	esc_html__( 'None', 'autlines' ),
								1	=>	esc_html__( 'One columns', 'autlines' ),
								2	=>	esc_html__( 'Two columns', 'autlines' ),
								3	=>	esc_html__( 'Three columns', 'autlines' ),
                                4	=>	esc_html__( 'Four columns', 'autlines' ),
                                5	=>	esc_html__( 'Five columns', 'autlines' ),
							),
						'size' => 'thin',
						'depth' => '0',
						'class' => 'autlines-show-only-depth-0 autlines-columns-limit',
					),
			);
		}
		...
		private function _add_filters() {
			...
			add_action('admin_enqueue_scripts', array( $this, 'autlines_mega_menu_admin_scripts' ), 80);
		}
		
		
 
		function autlines_mega_menu_admin_scripts() {
			wp_enqueue_media();
		}
		...
		public function apply_edit_walker_class( $walker, $menu_id ) {
			return AUTLINES_EDIT_MENU_WALKER_CLASS;
		}
	}
}
