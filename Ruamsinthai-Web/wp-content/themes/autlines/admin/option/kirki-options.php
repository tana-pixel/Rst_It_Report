<?php
if (!function_exists('autlines_customize_register'))
{
    function autlines_customize_register($wp_customize)
    {
...
add_action('customize_register', 'autlines_customize_register');

if (!function_exists('initial_kirki_options') && class_exists('FL_Options'))
{

    function initial_kirki_options()
    {
    FL_Options::add_config(array(
...
if (!function_exists('autlines_get_theme_mod')) {
    function autlines_get_theme_mod($name = null, $use_acf = null, $postId = null, $acf_name = null)
    {
        $value = null;
        ...
        if ($use_acf) {
            $value = autlines_get_metabox($acf_name ? $acf_name : $name, $postId);
        }

        // get value from options
        if (($value === null || $value === 'default')) {
            if (class_exists('FL_Options')) {
                $value = FL_Options::get_option($name);
            }
        }

        $value = apply_filters('autlines_filter_get_theme_mod', $value, $name);
        return $value;
    }
}
// get metabox
if (!function_exists( 'autlines_get_metabox' )):
    function autlines_get_metabox($name = null, $postId = null)
    {
        $value = null;
